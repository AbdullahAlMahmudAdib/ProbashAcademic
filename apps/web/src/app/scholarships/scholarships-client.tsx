...
function ScholarshipsContent() {
  ...
  const [toggleNoIelts, setToggleNoIelts] = useState(false);
  const [toggleStudyGapOk, setToggleStudyGapOk] = useState(false);
  const [toggleFullyFunded, setToggleFullyFunded] = useState(false);
  const [toggleNoAppFee, setToggleNoAppFee] = useState(false);
  const [toggleDeadlineSoon, setToggleDeadlineSoon] = useState(false);
  ...
  const applyFiltersAndSort = (list: Scholarship[]) => {
    ...
    let result = list.filter((s) => {
      ...
      if (toggleNoIelts && !(s.tags ?? []).some((t) => t.toLowerCase().includes("no-ielts") || t.toLowerCase().includes("no ielts"))) return false;
      if (toggleStudyGapOk && !(s.tags ?? []).some((t) => t.toLowerCase().includes("study-gap") || t.toLowerCase().includes("gap"))) return false;
      if (toggleFullyFunded && s.funding_type !== "full") return false;
      if (toggleNoAppFee && !(s.tags ?? []).some((t) => t.toLowerCase().includes("no-app") || t.toLowerCase().includes("no application") || t.toLowerCase().includes("fee waiver") || t.toLowerCase().includes("free"))) return false;
      if (toggleDeadlineSoon && !isWithin30Days(s.deadline)) return false;
      if (!q) return true;
  ...
  }, [scholarships, searchTerm, selectedCountries, selectedFunding, selectedLevels, sortBy, toggleNoIelts, toggleStudyGapOk, toggleFullyFunded, toggleNoAppFee, toggleDeadlineSoon]);
  ...
  const clearFilters = () => {
    ...
    setToggleNoIelts(false); setToggleStudyGapOk(false); setToggleFullyFunded(false); setToggleNoAppFee(false); setToggleDeadlineSoon(false);
  };

  const hasActiveFilters = searchTerm || selectedCountries.length || selectedFunding.length || selectedLevels.length || sortBy !== "Deadline"
    || toggleNoIelts || toggleStudyGapOk || toggleFullyFunded || toggleNoAppFee || toggleDeadlineSoon;
  ...
  return (
    <div className={styles.page}>
      ...
      <main className={styles.main}>
        ...
        {/* Active filter chips */}
        {(selectedCountries.length > 0 || selectedFunding.length > 0 || selectedLevels.length > 0 || toggleNoIelts || toggleStudyGapOk || toggleFullyFunded || toggleNoAppFee || toggleDeadlineSoon) && (
        ...
        <div className={styles.toggleChips}>
          <button type="button" className={`${styles.toggleChip} ${toggleNoIelts ? styles.toggleChipActive : ""}`} onClick={() => setToggleNoIelts((v) => !v)}>
            {t("scholarships.noIelts")}
          </button>
          <button type="button" className={`${styles.toggleChip} ${toggleStudyGapOk ? styles.toggleChipActive : ""}`} onClick={() => setToggleStudyGapOk((v) => !v)}>
            {t("scholarships.studyGapOk")}
          </button>
          <button type="button" className={`${styles.toggleChip} ${toggleFullyFunded ? styles.toggleChipActive : ""}`} onClick={() => setToggleFullyFunded((v) => !v)}>
            {t("scholarships.fullyFunded")}
          </button>
          <button type="button" className={`${styles.toggleChip} ${toggleNoAppFee ? styles.toggleChipActive : ""}`} onClick={() => setToggleNoAppFee((v) => !v)}>
            {t("scholarships.noAppFee")}
          </button>
          <button type="button" className={`${styles.toggleChip} ${toggleDeadlineSoon ? styles.toggleChipActive : ""}`} onClick={() => setToggleDeadlineSoon((v) => !v)}>
            {t("scholarships.deadlineSoon")}
          </button>
        </div>
